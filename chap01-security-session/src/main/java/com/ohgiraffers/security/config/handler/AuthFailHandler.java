package com.ohgiraffers.security.config.handler;
...
@Configuration
public class AuthFailHandler extends SimpleUrlAuthenticationFailureHandler { // 사용자가 로긴 실패했을때 핸들러
    ...
    * */
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        if(exception instanceof BadCredentialsException){
            ...
            //보안 컨텍스트에 인증 객체가 존재하지 않거나 인증 정보가 없는 상태에서 보안처리된 리소스에 접근하는 경우 발생=>서버의 오류
        ...
        setDefaultFailureUrl("/auth/fail?message="+errorMessage); //->이 url에 대한 컨트롤러 필요함

        // 부모에 메서드를 호출하여 다음 로직을 수행하도록 하기 위함이다. => 다음 필터로 넘겨주는거..?
